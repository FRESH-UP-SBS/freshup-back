...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
        ...
        /**
         * 특정 회원이 주어진 기간 내에 완료한 청소 일정 횟수를 반환합니다.
         *
         * TB_SCHEDULE 에는 회원별 업무(청소) 일정이 기록됩니다.
         * - EVENT_DATE 가 해당 주 범위 안에 있고
         * - 일정이 실제로 완료된 것만 카운트 (완료 여부 판단 기준: EVENT_DATE <= 현재시각)
         *
         * ※ 완료 여부를 별도 상태 컬럼으로 관리하는 경우 WHERE 조건을 수정하세요.
         * 예) AND s.status = 'DONE'
         */
        @Query("""
                        SELECT COUNT(s)
                        FROM Schedule s
                        WHERE s.user.id = :userSeq
                        AND s.date >= :weekStart
                        AND s.date <= :weekEnd
                        AND s.date <= CURRENT_TIMESTAMP
                        """)
        int countCompletedScheduleByUserAndPeriod(
