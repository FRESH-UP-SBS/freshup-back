...
public interface PenaltyRepository extends JpaRepository<Penalty, Long> {
        ...
        @Query("""
                        ...
                        FROM Penalty p
                        JOIN FETCH p.user
                        WHERE (:assignees IS NULL OR p.user.id IN :assignees)
                        AND (:paymentStatus IS NULL OR :paymentStatus = 'ALL' OR p.adjustmentYn = :paymentStatus)
                        AND (:startDate IS NULL OR p.createdDate >= :startDate)
                        AND (:endDate IS NULL OR p.createdDate <= :endDate)
                        ORDER BY p.id desc
                        """)
        Page<Penalty> findAllWithFilter(
                        @Param("assignees") List<Long> assignees, // 담당자 ID 목록
                        @Param("paymentStatus") String paymentStatus, // 수납 여부 (ALL, Y, N)
                        @Param("startDate") LocalDate startDate, // 시작 날짜
                        @Param("endDate") LocalDate endDate, // 종료 날짜
